...
from util.unicsv import DictUnicodeReader
...
class SegmentationData(AbstractSegmentation):
    ...
    def __init__(self, directory, categories=None, require_all=False):
        ...
        with open(os.path.join(directory, 'index.csv')) as f:
            self.image = [decode_index_dict(r) for r in csv.DictReader(f, dialect=csv.excel)]
        ...
        with open(os.path.join(directory, 'label.csv')) as f:
            label_data = [decode_label_dict(r) for r in csv.DictReader(f, dialect=csv.excel)]
        ...
        for cat in self.category:
            with open(os.path.join(directory, 'c_%s.csv' % cat)) as f:
                c_data = [decode_label_dict(r) for r in csv.DictReader(f, dialect=csv.excel)]
...
class SegmentationPrefetcher:
    ...
    def form_caffe_tensors(self, batch, bgr_mean=None, global_labels=False):
        ...
        batchesFn = [[] for c in self.categories]
        for record in batch:
...
# import numpy
# ds = SegmentationData('/Users/yifan/Develop/robinterp/data/broden')
# pf = SegmentationPrefetcher(ds, categories=['image'], split=None, once=True, batch_size=1)
...
# print('unique textures seen', ';'.join(ds.name('texture', f) for f in found))
# fs, shape = ds.resolve_segmentation(ds.metadata(i))
# print('full segmentation', shape, ', '.join(fs.keys()))
# pf = SegmentationPrefetcher(ds, categories=['object'], once=True)
# print(ds.size())
