...
class resloc(nn.Module):
    def __init__(self, block, num_blocks, num_classes=1000, dataset = "imagenetLOC"):
        ...
        self.mp_fc = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
        self.layer1 = self._make_layer(block, 64, num_blocks[0], stride=1)
        self.layer2 = self._make_layer(block, 128, num_blocks[1], stride=1)
        self.layer3 = self._make_layer(block, 256, num_blocks[2], stride=1)
        self.layer4 = self._make_layer(block, 512, num_blocks[3], stride=1)

        self.conv2 = nn.Conv2d(512, 1024, kernel_size=5, stride=2, padding=2, bias=False)
        self.bn2 = nn.BatchNorm2d(1024)
        self.conv3 = nn.Conv2d(1024, 1024, kernel_size=5, stride=2, padding=2, bias=False)
        ...
        self.fc = nn.Linear(2 * 1024 * block.expansion, num_types)
