...
class reslocNM(nn.Module):
    def __init__(self, block, num_blocks, num_classes=1000, dataset = "imagenetLOC"):
        ...
        self.classLinear = nn.Sequential(
            nn.Linear(self.num_types, 512),
            nn.ReLU(True),
            nn.Dropout(),
            nn.Linear(512, 512),
            nn.Softmax(dim=1),
        )
        VGGcfg = [512, 512, 'M', 1024, 1024, 'M', 1024, 1024, 'M', 1024, 1024, 'MF']
    ...
    def forward(self, x, classVec, target):
        ...
        inFeat = self.layer4(self.layer3(self.layer2(self.layer1(inFeat))))
