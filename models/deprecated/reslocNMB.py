...
class RegLayer(nn.Module):
    def __init__(self):
        ...
        self.heatConv1 = nn.Conv2d(256, 256, kernel_size=3, stride=2, padding=1, bias=False)
        self.heatBN = nn.BatchNorm2d(256)
        self.heatConv2 = nn.Conv2d(256, 128, kernel_size=1, stride=1, padding=0, bias=False)
    ...
    def forward(self, x):
        ...
        heat = F.relu(self.heatBN(self.heatConv1(out)))
        heat = self.heatConv2(heat)
...
class reslocNM(nn.Module):
    ...
    def forward(self, x, classVec, target):
        ...
        regCenLoss = (reg3CenLoss) * 20
        regResLoss = reg3ResLoss
