...
class resnetTrainer():
    ...
    def train(self, trainLoader, epoch):
        ...
        for i, (inputData, targetData) in enumerate(trainLoader):
            ...
            loss, reg, heatmap, (regCenLoss, regResLoss) = self.model.forward(inputData, inputClassVec, targetData)
            ...
            iouScore = evaluation.IoU(reg[2].data.cpu().numpy(), targetData.data.cpu().numpy())
            log = 'Epoch: [%d][%d/%d] Time %1.3f Data %1.3f IoU %1.3f Err %1.4f\n' % (epoch, i, len(trainLoader), runTime, dataTime, iouScore, loss.data[0])
            self.logger['train'].write(log)
            self.progbar.update(i, [('Time', runTime), ('Loss', loss.data[0]), ('CenLoss', regCenLoss.data[0]), ('ResLoss', regResLoss.data[0]), ('IoU', iouScore)])

            if i <= self.opt.visTrain:
                visImg.append(inputData.data.cpu())
                for j in range(3):
                    visImg.append(reg[j].data.cpu())
                    visImg.append(heatmap[j].data.cpu())
    ...
    def visualize(self, visImg, epoch, split, postprocess, postprocessTarget, postprocessHeat):
        outputImgs = []
        for i in range(len(visImg) // 8):
            for j in range(self.opt.batchSize):
                img = postprocess()(visImg[8 * i][j].numpy())
                outputImgs.append(img)
                for k in range(3):
                    regResult = postprocessTarget()(visImg[8 * i + 2 * k + 1][j].numpy())
                    outputImgs.append(self.drawBox(img, regResult))
                    heatResult = postprocessHeat()(visImg[8 * i + 2 * k + 2][j].numpy())
                    outputImgs.append(heatResult)
                regResult = postprocessTarget()(visImg[8 * i + 7][j].numpy())
                outputImgs.append(self.drawBox(img, regResult))
        vis.writeImgHTML(outputImgs, epoch, split, 8, self.opt)
