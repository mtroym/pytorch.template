...
class resloc(nn.Module):
    def __init__(self, block, num_blocks, num_classes=1000, dataset = "imagenetLOC"):
        ...
        self.linear10 = nn.Linear(1024 * block.expansion, num_types)
    ...
    def forward(self, x):
        ...
        out = self.layer2(out)
        out = self.layer3(out)
        heats = self.layer4(out)
        out = F.relu(self.bn20(self.conv20(heats)))
        out = F.relu(self.bn30(self.conv30(out)))
        out = F.avg_pool2d(out, kernel_size=14)
