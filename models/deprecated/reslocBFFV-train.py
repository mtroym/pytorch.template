...
#             max_heat =  torch.sum(inFeat, dim=1, keepdim=True).expand(8,3,56,56)
    ...
    def test(self, valLoader, epoch):
        ...
        for i, (inputData, targetData) in enumerate(valLoader):
            ...
            start = time.time()
            inputData, inputClassVec = inputData
            inputData, inputClassVec = Variable(inputData), Variable(inputClassVec)
            
            target = Variable(targetData[:,0,:])
            ...
            loss, reg, (regCenLoss, regResLoss), classFeat, inFeat = self.model.forward(inputData, inputClassVec, target)
            runTime = time.time() - start
            ...
            iouScore, bestTarget = evaluation.ValIoU(reg.data.cpu().numpy(), targetData)
        ...
        for i in range(len(visImg) // 6):
            for j in range(self.opt.batchSize):
                ...
                if (split == 'train'):
                    ...
                    regResult = postprocessTarget()(visImg[4 * i + 3][j].numpy())
                    outputImgs.append(self.drawBox(img, regResult))
                else:
                    regResult = postprocessTarget()(visImg[4 * i + 2][j])
                    outputImgs.append(self.drawBox(img, regResult))
