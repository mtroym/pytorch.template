...
class resnetTrainer():
    ...
    def train(self, trainLoader, epoch):
        ...
        for i, (inputData, targetData) in enumerate(trainLoader):
            ...
            iouScore = evaluation.IoU(reg[1].data.cpu().numpy(), targetData.data.cpu().numpy())
    ...
    def visualize(self, visImg, epoch, split, postprocess, postprocessTarget, postprocessHeat):
        ...
        for i in range(len(visImg) // 6):
            for j in range(self.opt.batchSize):
                img = postprocess()(visImg[6 * i][j].numpy())
                outputImgs.append(img)
                for k in range(2):
                    regResult = postprocessTarget()(visImg[6 * i + 2 * k + 1][j].numpy())
                    outputImgs.append(self.drawBox(img, regResult))
                    heatResult = postprocessHeat()(visImg[6 * i + 2 * k + 2][j].numpy())
                    outputImgs.append(heatResult)
                regResult = postprocessTarget()(visImg[6 * i + 5][j].numpy())
                outputImgs.append(self.drawBox(img, regResult))
